...
Future<void> platlistDetail(String id) async {
  String postData = '''
    {
    "method":"POST",
  ...
  ObservableList<Song> _playlistSongs = ObservableList();
  List songs = json['playlist']['tracks'];
  for (var song in songs) {
    //读取歌手
    List artists = song['ar'];
    ...
    _playlistSongs.add(Song(song['name'], song['id'].toString(), alias_,
        artists_, album_)); //这里的Url默认是http而不是https，申必，，，
  }
  storePlaylistSongs.updatePlaylistSongs(_playlistSongs);
}
